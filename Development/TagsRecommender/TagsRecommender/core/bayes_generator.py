from TagsRecommender.core import tweetsextractor, bayes_classification, preprocessor
import json
import threading
import collections


class BayesHashtagsGenerator(threading.Thread):
    def __init__(self, screen_name, current_tweet,
                 number_of_hashtags=3, file_name=""):
        super().__init__()
    ...
    def get_recommendations(self):
        ...
        bayes = bayes_classification.BayesClassificator(tweet_words=current_tweet, all_tweets=tweets,
                                                        number_of_hashtags=self.__number_of_hashtags)
        return bayes.get_recommended_hashtags()
